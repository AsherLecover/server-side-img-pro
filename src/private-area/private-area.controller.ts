...
export class PrivateAreaController {
  ...
  @Get('get-pic/:id')
  getImgfile(@Res() res: any, @Param('id') id) {
    return res.sendFile(
      join(process.cwd(), `uploads/images-user-uploads/${id}.jpg`),
    );
  }

  @Get('getFile/:id')
  getfile(@Res() res: any, @Param('id') id) {
    return res.sendFile(
      join(process.cwd(), `uploads/images-user-profile/${id}.jpg`),
    );
  }
  ...
  @Delete('/:id')
  deleteImg(@Param('id') params: Request, @Req() request: Request) {
  ...
  async editImgDetails(
    ...
    return await this.privateAreaService.editImgDetails(
      params['id'],
      img,
      params['userId'],
    );
  ...
  @Patch('')
  setCardProfile(@Body() data) {
    console.log('dataaaa:', data);
    return this.privateAreaService.setCardProfile(
      data.data,
      data.userId,
      data.colomnName,
    );
